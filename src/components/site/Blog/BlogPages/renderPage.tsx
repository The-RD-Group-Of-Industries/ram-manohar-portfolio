/**
 * Renders a blog page component with the provided blog data.
 *
 * @param {Object} props - The component props.
 * @param {Blog | null} props.data - The blog data to be displayed.
...
export default function BlogRP({ data }: { data: Blog | null }) {
  ...
  return (
    <div className='px-[5vw] font-satoshi bg-gradient-to-b from-[#1E1E20] to-[#0E0E11] overflow-hidden text-gray-100 min-h-screen'>
      ...
      >
        ...
        {data?.trending && 
          <p 
            className="bg-gradient-to-r from-[#407cff8f] to-white bg-clip-text text-left text-transparent text-[26px] tracking-tight font-mono font-bold md:text-[4vh] lg:text-[7vh] py-2 ml-5"
      ...
      >
        ...
        <div 
          className="text-[16px] md:text-base md:py-5 md:px-10 py-3 px-5 bg-[#ffffff18] rounded-md md:rounded-lg"
          data-aos="fade-up" // AOS effect
          dangerouslySetInnerHTML={{ __html: data?.desc || '' }} // Render HTML description
        />
      </div>
    </div>
  );
